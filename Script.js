...
function animateIcon(iconId, inputId, resultId) {
    ...
    setTimeout(() => {
        ...
        flyIcon.style.transform = 'translate(0,0)';
    }, 800);
}
...
function convertCurrency() {
    ...
    if(isNaN(val)) return alert("Enter a number!");
    const rates = { usd:1, qar:3.64, pkr:284, eur:0.92 };
    const result = val * rates[to]/rates[from];
    document.getElementById('currencyResult').innerHTML = `${getIcon('currency')} ${result.toFixed(2)} ${to.toUpperCase()}`;
